...
mod tests {
    ...
    #[test]
    fn tokenize_match_equal() {
        let tokens = tokens_from_str("==>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchEqual, "==>");
    }

    #[test]
    fn tokenize_match_not_equal() {
        let tokens = tokens_from_str("!==>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchNotEqual, "!==>");
    }

    #[test]
    fn tokenize_match_less_than() {
        let tokens = tokens_from_str("<=>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchLessThan, "<=>");
    }

    #[test]
    fn tokenize_match_less_than_or_equal() {
        let tokens = tokens_from_str("<==>");
        assert_token(
            tokens.get(0).unwrap(),
            TokenType::MatchLessThanEqual,
            "<==>",
        );
    }

    #[test]
    fn tokenize_match_greater_than() {
        let tokens = tokens_from_str(">=>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchGreaterThan, ">=>");
    }

    #[test]
    fn tokenize_match_greater_than_or_equal() {
        let tokens = tokens_from_str(">==>");
        assert_token(
            tokens.get(0).unwrap(),
            TokenType::MatchGreaterThanEqual,
            ">==>",
        );
    }

    #[test]
    fn tokenize_match_keys_equal() {
        let tokens = tokens_from_str(":=>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchKeysEqual, ":=>");
    }
    ...
    #[test]
    fn tokenize_match_values_equal() {
        let tokens = tokens_from_str("$=>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchValuesEqual, "$=>");
    }
    ...
    #[test]
    fn tokenize_match_contains() {
        let tokens = tokens_from_str("~=>");
        assert_token(tokens.get(0).unwrap(), TokenType::MatchContains, "~=>");
    }
